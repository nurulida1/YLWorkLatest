...
namespace YLWorks.Controller
{
    ...
    public class SystemModuleController : ControllerBase
    {
        ...
        public SystemModuleController(AppDbContext context, IHubContext<NotificationHub> hub)
        {
            _context = context;
            _hub = hub;
  ...
  string? includes = null)
        {
            try
            {
                ...
                {
                    var parameter = Expression.Parameter(typeof(SystemModule), "u");
                    ...
                    {
                        var lambda = Expression.Lambda<Func<SystemModule, bool>>(finalExpression, parameter);
      ...
      .ToList();
        ...
        public async Task<ActionResult<SystemModule>> AddSystemModule([FromBody] CreateSystemModuleRequest request)
        {
            ...
            try
            {
                ...
                var result = await _context.SystemModules
                    .Where(d => d.Id == systemModule.Id)
                    .Select(d => new SystemModule
                    {
                        Id = d.Id,
                        Name = d.Name,
                        Code = d.Code
                ...
                await _hub.Clients.All.SendAsync("SystemModuleAdded", systemModule);

                return Ok(result);
            }
            catch (Exception)
            {
                return StatusCode(500, new { Error = "Failed to add system module." });
            }
        }

        [HttpPut("Update")]
        public async Task<ActionResult<SystemModule>> UpdateSystemModule([FromBody] UpdateSystemModuleRequest request)
        {
            if (!ModelState.IsValid)
                return BadRequest(ModelState);

            var module = await _context.SystemModules.FindAsync(request.Id);
            if (module == null)
                return NotFound(new { Error = "System Module not found." });
           ...
           .FirstAsync();
            ...
            {
                return StatusCode(500, new { Error = "Failed to update system module." });
            }
        }

        [HttpDelete("Delete")]
        public async Task<ActionResult> DeleteSystemModule([FromQuery] Guid id)
        {
            ...
            try
            {
                ...
                // Optional: Notify via SignalR
                await _hub.Clients.All.SendAsync("SystemModuleDeleted", id);

                return Ok(new { Message = "System module deleted successfully." });
            }
            catch (Exception ex)
            {
                return StatusCode(500, new { Error = "Failed to delete system module." });
            }
        }
    }
}
