...
import { AccessPermissionDto } from '../../models/AccessPermission';
...
@Component({
  ...
  template: `<div class="w-full flex flex-col p-5">
      ...
      >
        ...
        /
        <div class="text-gray-700 font-semibold">Access Permission</div>
      </div>
      <div
        ...
        <div class="flex flex-row items-center justify-between">
          <div class="flex flex-col">
            ...
            <div class="text-gray-500 text-[15px]">
              Create, edit, and manage access permission
            </div>
          </div>
  ...
  styleUrl: './accessPermission.less',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class AccessPermission implements OnInit, OnDestroy {
  @ViewChild('fTable') fTable?: Table;

  private readonly loadingService = inject(LoadingService);
  private readonly messageService = inject(MessageService);
  private readonly accessPermissionService = inject(AccessPermissionService);
  private readonly cdr = inject(ChangeDetectorRef);
  protected ngUnsubscribe: Subject<void> = new Subject<void>();

  PagingSignal = signal<PagingContent<AccessPermissionDto>>(
    {} as PagingContent<AccessPermissionDto>,
  );
  ...
  search: string = '';
  title: string = 'Add New Access Permission';
  name: string | null = null;
  accessPermissionId: string | null = null;
  menuItems: MenuItem[] = [];
  ...
  ActionClick(data: AccessPermissionDto | null, action: string) {
    if (action === 'Delete' && data) {
        ...
        .subscribe({
          next: (res: any) => {
            ...
            this.messageService.add({
              severity: 'success',
              summary: 'Deleted',
              detail: res?.message || 'Access Permission deleted successfully',
            });
          ...
          error: (err: any) => {
            ...
            this.messageService.add({
              ...
              detail:
                err.error?.error ||
                err.error?.message ||
                'Failed to delete access permission',
            });
          },
        });
    } else {
      ...
      if (this.isUpdate) {
        this.accessPermissionId = data?.id || null;
  ...
  SaveAccessPermission() {
    if (!this.name) {
      return this.messageService.add({
        severity: 'error',
        summary: 'Access Permission is required',
        detail: '',
      });
    }

    const request$: Observable<any> = this.isUpdate
      ? this.accessPermissionService.Update({
          id: this.accessPermissionId!,
          name: this.name,
        })
      : this.accessPermissionService.Create({
          name: this.name,
        });
      ...
      next: (res) => {
        ...
        if (res) {
          this.messageService.add({
            severity: 'success',
            summary: 'Success',
            detail: `Access Permission ${this.name + ' has been ' + this.isUpdate ? 'updated' : 'created'} successfully`,
          });
