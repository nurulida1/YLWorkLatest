...
import { LocationInventoryService } from '../../../services/LocationCategoryService';
...
@Component({
  ...
  template: `<div class="w-full flex flex-col p-5">
      <div class="flex flex-row items-center gap-1 text-gray-500 tracking-wide">
        ...
        /
        <div class="text-gray-700 font-semibold">Locations</div>
      </div>
      <div
      ...
      >
        <div class="flex flex-row items-center justify-between">
          <div class="flex flex-col">
            <div class="text-[20px] text-gray-700 font-semibold">Locations</div>
  ...
  styleUrl: './locationInventory.less',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class LocationInventory implements OnInit, OnDestroy {
  @ViewChild('fTable') fTable?: Table;

  private readonly cdr = inject(ChangeDetectorRef);
  private readonly loadingService = inject(LoadingService);
  private readonly messageService = inject(MessageService);
  private readonly locationInventoryService = inject(LocationInventoryService);
  ...
  ActionClick(data: BaseOption | null, action: string) {
    if (action === 'Delete' && data) {
      ...
      this.title = this.isUpdate ? 'Edit Location' : 'Add New Location';
  ...
  SaveLocation() {
    ...
    const request$: Observable<any> = this.isUpdate
      ? this.locationInventoryService.Update({
          id: this.id!,
          name: this.name,
        })
      : this.locationInventoryService.Create({
          name: this.name,
        });
      ...
      next: (res) => {
        ...
        if (res) {
          this.messageService.add({
            severity: 'success',
            summary: 'Success',
            detail: `Location: ${this.name} has been ${this.isUpdate ? 'updated' : 'created'} successfully`,
          });
